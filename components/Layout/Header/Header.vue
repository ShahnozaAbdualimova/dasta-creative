...
<header class="w-full fixed top-0 bg-dark-100 z-50">
  <div class="container flex-center-between py-6">
    ...
    <NuxtLinkLocale to="/">
      <img src="/images/logo.svg" alt="logo" class="max-w-[170px] w-full h-full" />
    </NuxtLinkLocale>
    <div class="flex-y-center gap-7">
      <div class="flex-y-center gap-9 max-sm:hidden">
        <NuxtLinkLocale v-for="item in menu" :key="item.name" :to="item.path">
          <span class="text-base leading-130 font-semibold transition-300 hover:text-blue">{{ item.name }}</span>
        </NuxtLinkLocale>
      </div>
      <div class="w-px h-8 bg-white/30 max-sm:hidden" />
      <LayoutHeaderLanguageSwitcher />
      <NuxtLinkLocale to="/contact" class="max-sm:hidden">
        <BaseButton variant="secondary" :text="$t('discuss_the_project')" size="sm" />
      </NuxtLinkLocale>
    </div>
