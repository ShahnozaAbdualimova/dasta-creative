  ...
  <div v-if="showSection" class="relative">
    <section class="relative overflow-hidden h-auto bg-brand-black">
      ...
      :loading="loading"
        data-aos="fade-up" 
        is-visible />
      <MainSupportNumberOfProjectsMobile :projects="stats" data-aos="fade-up"
        class="md:hidden mt-0 my-10 md:mt-16 md:mb-96" is-visible />
      <div v-if="!loading"
      ...
      <div class="container py-8 md:py-8 grid gap-6 grid-cols-1 md:grid-cols-3">
        <CommonSectionHeaderTitle data-aos="fade-up" class="flex flex-col gap-2 justify-between"
          :title="$t('what_we_can_do')" :subtitle="$t('what_we_can_do_text')">
          <div data-aos="fade-up" class="flex-y-center gap-5">
            <BaseButton variant="secondary" class="w-full" :text="$t('discuss_project')"
        ...
        <template v-if="!loading">
          <CommonCardSupport v-for="(item, key) in supports" :key :loading="false" v-bind="item" />
        </template>
        <template v-if="loading">
