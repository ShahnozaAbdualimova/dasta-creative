...
<div class="pb-8 pt-16 md:py-16 container">
  ...
  <div class="flex-y-center flex-wrap gap-5 mt-6"  data-aos="fade-right">
    <button class="py-2 px-6 rounded-lg border border-blue transition-300 text-sm leading-4 text-white font-bold hover:text-blue" v-for="(item,index) in categories" :key="index" @click="active = item.id" :class="{'!text-dark-100 !bg-white !border-white': active === item.id}">
      {{ item?.title }}
    </button>
    <NuxtLinkLocale to="/projects">
      <BaseButton :text="$t('all_projects')" size="sm" >
        <template #suffix>
  ...
  <Transition name="fade">
    <div v-if="count > projects?.length && showMore" class="flex-center mt-8" @click="loadMore">
